$(document).ready(function(){
        $('#id__fields').change(field_selected);
...
function get_fal(view_id){
    $.ajax({url:'/admin/view/'+view_id+'/get_fal/',type:'get',data:{},success:load_fal});
}
...
function update_field_perm(fid,pid){
    view.fal[fid][pid] = $('#fperm_'+fid+'_'+pid).val();
}

function add_permission(fid,ele,id,name){
    view.fal[fid][id] = view.persona_perms[id];
    $(ele).append(
        $('<tr id="frow_'+fid+'" ><td>'+name+'</td><td><select name="fperm_'+fid+'_'+id+'" id="fperm_'+fid+'_'+id+'" onchange="update_field_perm(\''+fid+'\',\''+id+'\')'+'" >'+view.perm_options+'</select></td><td><a href="javascript://" onclick="javascript:$(\'#frow_'+fid+'\').remove();" ><img src="/static/base/images/icon_delete.png" alt="Delete" class="mid_align" /></a></td></tr>')
    );
    $('#fperm_'+fid+'_'+id).selectOptions(view.persona_perms[id]);
}

function field_selected(){
    $('#sel_fields').html("");
    view.fal = {};
    $('#id__fields :selected').each(function(i,s){
...
function apply_permissions(){
    if($('#id__fields :selected').length < 1){
    ...
    $('#id__fields :selected').each(function(i,selected){
...
function send_view(add,vid){
    ...
    if(add == true){
        $.ajax({url:'/admin/view/new/',type:'post',data:{'fal':view.get_fal(),'name':$('#id_name').val(),'description':$('#id_description').val(),'fields':$('#id__fields').val()},success:view_updated});
    }
    else{
        $.ajax({url:'/admin/view/'+vid+'/edit/',type:'post',data:{'fal':view.get_fal(),'name':$('#id_name').val(),'description':$('#id_description').val(),'fields':$('#id__fields').val()},success:view_updated});
    }
}

function View(){

    this.perm_options = "<option value='--'>=== Select ===</option>"+
